...
import define1 from "./e93997d5089d7165@2248.js";

export default function define(runtime, observer) {
  const main = runtime.module();
  main.variable(observer()).define(["md"], function(md){return(
md`# Seguimiento Diario`

)});
  main.variable(observer("viewof sorting")).define("viewof sorting", ["select"], function(select){return(
select({title: 'Ordenado por', options:["tipoAplicacion","time"], value:"time"})
)});
  main.variable(observer("sorting")).define("sorting", ["Generators", "viewof sorting"], (G, _) => G.input(_));
  main.variable(observer("chart")).define("chart", ["sorting","dataByAplicacion","data","d3","color","DOM","width","height","margin","createTooltip","y","getRect","getTooltipContent","axisTop","axisBottom"], function(sorting,dataByAplicacion,data,d3,color,DOM,width,height,margin,createTooltip,y,getRect,getTooltipContent,axisTop,axisBottom)
{

  let filteredData;
  if(sorting !== "time") {
    filteredData = [].concat.apply([], dataByAplicacion.map(d=>d.values));
  } else { 
    filteredData = data.sort((a,b)=>  a.start-b.start);
  }  

  filteredData.forEach(d=> d.color = d3.color(color(d.tipoAplicacion)))
  ...
  if (!parent) {
    ...
    civs.data(filteredData, d=>d.objeto)
      .transition()
      // .delay((d,i)=>i*10)
      .ease(d3.easeCubic)
...
return `<b>${d.objeto}</b>
<br/>
<b style="color:${d.color.darker()}">${d.tipoAplicacion}</b>
<br/>
...
d3.scaleLinear()
      .domain([d3.min(data, d => 0), d3.max(data, d => 2400)])
...
d3.nest().key(d=>d.timeline).entries(data)
  ...
  main.variable(observer("dataByAplicacion")).define("dataByAplicacion", ["d3","data"], function(d3,data){return(
d3.nest().key(d=>d.tipoAplicacion).entries(data)
)});
...
d3.nest().key(d=>d.tipoAplicacion).entries(data).map(d=>d.key)
)});
